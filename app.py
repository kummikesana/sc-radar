...
def analyze_risk(target: str, company_name: str, company_context: str,
                  news_digest: str, gemini_key: str) -> dict:
    ...
    prompt = f"""You are a senior supply chain risk intelligence analyst.
...
{{
  ...
  "recommended_actions": ["<action 1 specific to {company_name if company_name != 'Custom scan' else 'the company'}>",
                           "<action 2>", "<action 3>"],
...
<div class="radar-header">
  ...
  <div class="radar-header-right">NewsAPI + Gemini AI<br>Free tier</div>
</div>
""", unsafe_allow_html=True)
...
if run_scan and target:
    ...
    gk = st.session_state.get("gemini_key", "")
    if not nk or not gk:
...
st.markdown("""
<div class="footer">
  SC Disruption Radar · Streamlit + NewsAPI + Gemini AI · HEC Paris MiM · IIT Kharagpur
</div>""", unsafe_allow_html=True)
